...
app = Flask(__name__)
# Para testes, uma chave secreta pode ser definida diretamente.
app.secret_key = 'uma-chave-secreta-para-testes' 

# --- 2. CONFIGURAÇÃO DO BANCO DE DADOS (ALTERE COM SEUS DADOS LOCAIS) ---
# Em vez de ler de um arquivo .env, colocamos as credenciais aqui para facilitar os testes.
DB_CONFIG = {
    'host': 'localhost',
    'user': 'root',
    'password': 'root', # Lembre-se de colocar sua senha aqui
...
# --- 3. DECORATORS DE AUTENTICAÇÃO E AUTORIZAÇÃO (sem alterações) ---
...
# ROTAS PRINCIPAIS (LOGIN, LOGOUT, DASHBOARD, ESCOLHA)
@app.route('/')
def home():
...
def login_or_register():
    ...
    try:
        ...
        print(f"ERRO NA ROTA /login: {e}") # Imprime o erro no terminal
...
if __name__ == '__main__':
    # O debug=True é ótimo para desenvolvimento, pois mostra erros detalhados no navegador
    # e reinicia o servidor automaticamente quando você salva o arquivo.
